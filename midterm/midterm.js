...
function drawNumber(number, buffer) {
  gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
  gl.vertexAttribPointer(vPosition, 2, gl.FLOAT, false, 0, 0);
  gl.drawElements(gl.TRIANGLES, numbers[number][0], gl.UNSIGNED_BYTE, numbers[number][1]);
}
function render() {
  ...
  if (number < 10 && document.getElementById("inp_number").value != "") {
    drawNumber(number, bufferNum1);
    drawNumber(0, bufferNum2);
  }
  else if (number < 100) {
    digit1 = number % 10; //units digit
    digit2 = Math.floor(number / 10); //tens digit

    drawNumber(digit1, bufferNum1);
    drawNumber(digit2, bufferNum2);
  }
  else {
    //if number is bigger than 100, displays 00
    drawNumber(0, bufferNum1);
    drawNumber(0, bufferNum2);
  }
